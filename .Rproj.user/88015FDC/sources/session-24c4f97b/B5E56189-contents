...
#free parameter: lamda_w
delta_w = function(lamda_w, error, a_hid){
  del_w = matrix(0, length(error), length(a_hid)) # Cat x Examplar, same to w
  for(k in 1:length(error)){
    for(j in 1:length(a_hid)){
    del_w[k,j] =  lamda_w*(error[k])*(a_hid[j])
    }
  }
  return(del_w)
}
...
#=================#
ALCOV_trial = function(c, phi, lw, la, alpha, w, h,q, r, a_in,fb, t_type){
    ...
    e = error(t, a_o)
    dw = delta_w(lw, e, a_hid)
    bp = bp_error(a_hid, e, w)
    da = delta_alpha(la, c, h, a_in, bp) #YJ
    nw = w+dw
    nalpha = alpha + da 
